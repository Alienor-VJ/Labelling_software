...
class ImageSegmentationApp:
    ...
    def compute_barycenter(self, points):
        # Compute the centroid (barycenter) of a polygon
    ...
    def update_area_tables(self):
        ...
        for category, shapes in self.categories.items():
            for shape in shapes:
                ...
                barycenter = self.compute_barycenter(shape)
                distance = self.compute_distance(barycenter)
                ellipse_ratio = self.compute_ellipse_ratio(shape)

                # Display the values in the correct table
                table_entry = f"shape: Area: {area:.2f} pixels², Dist: {distance:.2f} pixels, Ellipse Ratio: {ellipse_ratio:.2f}\n"
    ...
    def save_results_to_csv(self):
        if not self.img_path:
            print("No image loaded, cannot save results.")
            return

        # Extract filename without extension
        img_filename = os.path.splitext(os.path.basename(self.img_path))[0]
        csv_filename = f"{img_filename}_results.csv"

        with open(csv_filename, mode='w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["Shape Number", "Type", "Mean Area", "Ellipse Ratio", "Distance to Center"])

            shape_counter = 1
            for category, shapes in self.categories.items():
                for shape in shapes:
                    area = self.compute_area(shape)
                    ellipse_ratio = self.compute_ellipse_ratio(shape) or "N/A"
                    distance = self.compute_distance(shape[0])
                    writer.writerow([shape_counter, category, f"{area:.2f}",
                                     f"{ellipse_ratio:.2f}" if isinstance(ellipse_ratio, float) else ellipse_ratio,
                                     f"{distance:.2f}"])
                    shape_counter += 1
